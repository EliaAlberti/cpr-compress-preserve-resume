...
- **/compress** runs once at end of session: it summarises the conversation, asks the user what to preserve, writes the structured log. Most of the cost is inference (summarisation), not file I/O. The raw conversation archive is appended by `scripts/dump_transcript.py` from the Claude Code transcript on disk, so it costs zero model output tokens. The figures above assume that. Before v1.2.0 the model re-typed the archive, a cost this model never counted: measured across 42 real sessions of 20+ turns, it was ~2,900 / ~4,300 / ~9,900 output tokens per `/compress` at the lower quartile / median / upper quartile (largest ~640,000). Adding those to the With-CPR totals gives 6,600 / 11,800 / 22,900, so the real pre-v1.2.0 saving was net negative / 30% / 44% rather than 24% / 55% / 68%. v1.2.0 is what makes the modelled figures hold.
